import processing.core.*; import processing.candy.*; import processing.xml.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; import javax.sound.midi.*; import javax.sound.midi.spi.*; import javax.sound.sampled.*; import javax.sound.sampled.spi.*; import java.util.regex.*; import javax.xml.parsers.*; import javax.xml.transform.*; import javax.xml.transform.dom.*; import javax.xml.transform.sax.*; import javax.xml.transform.stream.*; import org.xml.sax.*; import org.xml.sax.ext.*; import org.xml.sax.helpers.*; public class transparent_polychrome_star_cluster_2d_sequential extends PApplet {
...
class Form
{
	...
	public void draw ()
	{
		...
		for (int i = 0; i < 5; i++)
		{
			float theta = (TWO_PI / 10.0f) * (i *2);
			vertex (0.0f - (size / 4.0f) * sin (theta), 
			        0.0f + (size / 4.0f) * cos (theta));
			theta += (TWO_PI / 10.0f);
			vertex (0.0f - (size / 2.0f) * sin (theta), 
			        0.0f + (size / 2.0f) * cos (theta));
		}
		endShape ();
		popMatrix ();
	}
...
class Animation
{
  	...
  	if (side_length > size)
	{
  	...
  	form.x = x * (1.0f / (scale_factor + 0.00001f));
	form.y = y * (1.0f / (scale_factor + 0.00001f));
...
public void setup ()
{
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "transparent_polychrome_star_cluster_2d_sequential" });  }}